...
я часто записываю голосовые сообщения и пользуюсь диктофоном. а так же пишу документы и в блоге.  Хочу иметь как медиа материалы, так и их текстовую форму т.к. по ней лучше работает поиск и иногда это упрощает набор текста. 

Данные модели обучены на  dataset "аудиокнигах" т.е. идеальный входной звук это в тишине выразительное чтение диктора один одновременно звучащий голос - для распознавания с диктофона этот dataset хорошо подходит.
...
Можно использовать в пакетном режиме через `xargs + sh + parallel `. Например так: 
